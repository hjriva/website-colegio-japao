    ...
    btnCancelar.classList.add("btn-editar-cancelar");
      ...
      const wrapperBotoes = ev.querySelector("#wrapper-botoes");
      wrapperBotoes.style.flexDirection = 'row';
      wrapperBotoes.style.justifyContent = 'flex-end';
      wrapperBotoes.append(btnSalvar, btnCancelar);
...
btnExcluirQuiz.addEventListener("click", async () => {
    ...
    const res = await fetch("/excluirQuiz", {
        method: "POST",
        headers: { "Content-type": "application/json" },
        body: JSON.stringify({ idQuiz: quiz.id, idAtividade: quiz.atividade_id }),
    });
    const data = await res.json();
    if (data.sucesso) {
        container.innerHTML = "<p>Quiz excluído.</p>";
    }
});
